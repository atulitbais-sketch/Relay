...
function AIChat() {
  ...
  async function handleSend() {
    ...
    setIsTyping(true)
    try {
      const response = await sendMessage('default', text, 'nexora')
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        role: 'assistant',
        content: response.content,
        sources: (response.sources || []).map(source => source.document_name || source.memory_id).filter(Boolean),
        time: new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }),
      }])
    } catch (error) {
      setMessages(prev => [...prev, {
        id: Date.now() + 1,
        role: 'assistant',
        content: `I couldn't reach the Relay backend. ${error.message}`,
        time: new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }),
      }])
    } finally {
      setIsTyping(false)
